...
export class GetCycleStatusCommand extends BaseStatusCommand {
  ...
  public async execute(cycleID: string, options: GetStatusOptions = this.defaultOptions): Promise<Status> {
    ...
    const [cycleCheckIns, progresses, confidences] = await this.unzipKeyResultGroup(filteredKeyResults)
  ...
  private async getKeyResultsFromCycle(cycleID: string, options: GetStatusOptions): Promise<KeyResult[]> {
    const filters = {
      keyResult: {
