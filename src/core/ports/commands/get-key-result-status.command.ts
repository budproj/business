...
export class GetKeyResultStatusCommand extends BaseStatusCommand {
  ...
  public async execute(
    ...
    const keyResult =
      typeof keyResultOrID === 'string' ? await this.core.keyResult.getFromID(keyResultOrID) : keyResultOrID
    const latestCheckIn = await this.core.keyResult.getLatestCheckInForKeyResultAtDate(keyResult.id, options.date)
