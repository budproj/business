...
> {
  ...
  async canUserReadForSelf(
    ...
    const selectedConfidenceReports = await this.repository.find(selector)
    const canUserRead = selectedConfidenceReports.every(
      (confidenceReport) => confidenceReport.userId === user.id,
    )
