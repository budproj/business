...
export const InstructionBuilder = () => {
  ...
  const mapping = (): ChatCompletionRequestMessage => {
    return toMessage('system', [
      ...
      `O título do key-result está delimitado por <${Segments.KeyResult}> e </${Segments.KeyResult}>`,
      ...
      `O objetivo ao qual o key-result está vinculado está delimitado por <${Segments.Objective}> e </${Segments.Objective}>`,
      ...
      `A meta do key-result está delimitada por <${Segments.Goal}> e </${Segments.Goal}>`,
      // `The key-result goal is delimited by <${Segments.Goal}> and </${Segments.Goal}>`,
      `O prazo do key-result está delimitado por <${Segments.Deadline}> e </${Segments.Deadline}>`,
      // `The key-result deadline is delimited by <${Segments.Deadline}> and </${Segments.Deadline}>`,
      `Comentários da discussão estão delimitados por <${Segments.Comment}> e </${Segments.Comment}>`,
      // `Discussion comments are delimited by <${Segments.Comment}> and </${Segments.Comment}>`,
      `Check-ins sobre o progresso estão delimitados por <${Segments.Progress}> e </${Segments.Progress}>`,
      // `Check-ins about progress are delimited by <${Segments.Progress}> and </${Segments.Progress}>`,
      `Tarefas da estão delimitadas por <${Segments.Task}> e </${Segments.Task}>`,
      // `Checklist tasks are delimited by <${Segments.Task}> and </${Segments.Task}>`,
      `O usuário irá informar o formato da resposta que quer receber através de <${Segments.Response}> e </${Segments.Response}>`,
