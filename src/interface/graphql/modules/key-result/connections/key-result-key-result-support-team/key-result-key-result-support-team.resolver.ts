...
import { KeyResultSupportTeamCreateRequest, KeyResultSupportTeamRemoveRequest } from './requests/key-result-add-support-team.request'
...
> {
  ...
  @GuardedMutation(KeyResultGraphQLNode, 'key-result:update', { name: 'addUserAsSupportTeamToKeyResult' })
  protected async addUser(
    ...
    await this.corePorts.dispatchCommand<void>('add-user-to-key-result-support-team', request.data.keyResultId, request.data.userId)

    return keyResult
  }

  @GuardedMutation(KeyResultGraphQLNode, 'key-result:update', { name: 'removeUserAsSupportTeamToKeyResult' })
  protected async removeUser(
    ...
    await this.corePorts.dispatchCommand<void>('remove-user-to-key-result-support-team', request.data.keyResultId, request.data.userId)
